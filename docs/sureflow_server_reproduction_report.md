...
| 항목 | 논문 조건 | 실습에서 맞춘 값 |
|---|---:|---:|
| Epoch | 200 | 실험적으로 100 epoch 우선 수행 |
...
폴더명은 `LO_E400_B256_TS100k`로 생성되었으나, 실습 중 epoch 설정을 100으로 바꿔 학습을 돌린 이력이 있다. 따라서 정확한 epoch 재현 비교를 위해서는 checkpoint 내부 epoch 정보 및 학습 로그를 함께 확인할 필요가 있다.
...
1. 논문과 완전히 동일한 training protocol이 아닐 수 있음
   - 논문은 200 epochs, batch size 256 조건을 사용한다.
   - 본 실습에서는 epoch 100 설정 및 기존 폴더명 `E400` 등 설정 변경 이력이 섞여 있어 정확한 학습 조건 검증이 필요하다.
...
현재 결과는 논문 성능 재현이라기보다는, 논문 코드 기반 서버 재현 실습 및 troubleshooting 기록으로 보는 것이 적절하다.
...
최종적으로 서버에서 SUREFlow 학습과 평가를 실행하는 전체 pipeline을 구축하였고, LIBERO Spatial 기준 평균 성공률 0.302의 평가 결과를 얻었다.
